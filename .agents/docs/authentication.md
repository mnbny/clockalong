...
Clockify disconnect is local-only. Rust clears the saved API key and emits an auth-state change. The frontend must also reset Clockify's TanStack Query cache, clear the synced Clockify entry cache, and remove workspace-bound Clockify settings such as the default project, project override, and active Quick Timer association. This prevents API-key rotation from briefly showing the previous Clockify user's workspace, entries, or running timer.
